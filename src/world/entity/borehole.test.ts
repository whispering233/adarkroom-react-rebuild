...
describe('borehole entity', () => {
  it('exports a valid WorldEntity', () => {
    expect(boreholeEntity).toBeDefined()
    expect(boreholeEntity.type).toBe('borehole')
    expect(boreholeEntity.footprint).toEqual({ w: 1, h: 1 })
  })

  it('getDrawCommand returns correct bounds', () => {
    const { mask, explored } = makeMask([[5, 5]], [[5, 5]])
    const result = boreholeEntity.getDrawCommand(5, 5, 0, 0, false, mask, explored)
  ...
  it('getDrawCommand returns cell for visible footprint cell', () => {
    ...
    expect(result.cells[0]!.output.char).toBe('B')
  ...
  it('getDrawCommand uses dimmed fillStyle when isDimmed=true even if visible', () => {
    const { mask, explored } = makeMask([[5, 5]], [[5, 5]])
    const result = boreholeEntity.getDrawCommand(5, 5, 0, 0, true, mask, explored)
    expect(result.cells).toHaveLength(1)
  })

  it('getDrawCommand computes viewport-relative coordinates', () => {
    const { mask, explored } = makeMask([[10, 10]], [[10, 10]])
    const result = boreholeEntity.getDrawCommand(10, 10, 3, 7, false, mask, explored)
  ...
  it('onEnter returns setpiece.borehole eventId', () => {
    ...
    expect(result).toEqual({ eventId: 'setpiece.borehole' })
  })
})
