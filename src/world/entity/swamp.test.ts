...
describe('swamp entity', () => {
  it('exports a valid WorldEntity', () => {
    expect(swampEntity).toBeDefined()
    expect(swampEntity.type).toBe('swamp')
    expect(swampEntity.footprint).toEqual({ w: 1, h: 1 })
  })

  it('getDrawCommand returns correct bounds', () => {
    const { mask, explored } = makeMask([[5, 5]], [[5, 5]])
    const result = swampEntity.getDrawCommand(5, 5, 0, 0, false, mask, explored)
  ...
  it('getDrawCommand returns cell for visible footprint cell', () => {
    ...
    expect(result.cells[0]!.output.char).toBe('M')
  ...
  it('getDrawCommand uses dimmed fillStyle when isDimmed=true even if visible', () => {
    const { mask, explored } = makeMask([[5, 5]], [[5, 5]])
    const result = swampEntity.getDrawCommand(5, 5, 0, 0, true, mask, explored)
    expect(result.cells).toHaveLength(1)
  })

  it('getDrawCommand computes viewport-relative coordinates', () => {
    const { mask, explored } = makeMask([[10, 10]], [[10, 10]])
    const result = swampEntity.getDrawCommand(10, 10, 3, 7, false, mask, explored)
  ...
  it('onEnter returns setpiece.swamp eventId', () => {
    ...
    expect(result).toEqual({ eventId: 'setpiece.swamp' })
  })
})
